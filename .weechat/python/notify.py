...
#
#
# 2010-03-01, Dave Reisner <d@falconindy.com>
#     version 0.0.6: Fuck python-notify. Directly call notify-send.
...
weechat.register("notify", "lavaramano", "0.0.6", "GPL", "notify: A real time notification system for weechat", "", "")
...
def notify_show(data, bufferp, uber_empty, tagsn, isdisplayed, ishilight, prefix, message):
    ...
    if (weechat.config_get_plugin('smart_notification') == "on" and bufferp == weechat.current_buffer()):
        ...
        if (not prefix.contains("root")):
            show_notification(buffer, prefix + weechat.config_get_plugin('nick_separator') + message)

    return weechat.WEECHAT_RC_OK

def show_notification(chan,message):
    call(["notify-send", "-u", weechat.config_get_plugin('urgency'), "-t", weechat.config_get_plugin('timeout'), chan, message]);

# vim: autoindent et smarttab sw=4
